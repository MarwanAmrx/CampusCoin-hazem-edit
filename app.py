...
@app.route('/mobileAPIauth', methods=['POST','GET'])
...
def mobileAPI():
    ...
    if request.form['action'] == 'PAY':
            ...
            try:
                ...
                conn.commit()
                response  = call(request.form['CardNumber'] , request.form['ExpiryDate'].split("/")[1]  , request.form['ExpiryDate'].split("/")[0] , request.form['CVV'] , request.form['value'] )
                ...
                if json.loads(response.text)['statusDescription'] == 'Operation done successfully':
                    ...
                    c.execute("""INSERT INTO Transactions (FromT, ToT, cardNumber, CVV , expierDate,value,transactionType,actionTime) 
                              VALUES (?, ?, ?, ? , ?,?,?,?)"""
                              , (str(request.form['parent']), str(request.form['childEmail']), str(request.form['CardNumber']),  str(request.form['CVV']),  str(request.form['ExpiryDate']),  str(request.form['value']),'Bank Transaction',str(time)))
                    c.execute("UPDATE  Children  SET [value] =  [value] + ?   WHERE childEmail = ?",[(str(request.form['value'])),(str(request.form['childEmail']))])
                    conn.commit()
                    flash('')
                    ...
                    return 'Transaction failed or insufficient fund.'
            except:
               
                    
                return 'Transaction failed try again later.'
            
    if request.form['action'] == 'ViewChaild':
            conn = sqlite3.connect("data.db")
            c = conn.cursor()
            Data = c.execute("""SELECT * FROM Transactions WHERE FromT = ? or ToT = ? """, (str(request.form['childEmail']),str(request.form['childEmail']))).fetchall()
            

            return jsonify({"data":Data})
    if request.form['action'] == 'addChild':
            conn = sqlite3.connect("data.db")
            c = conn.cursor()
            Data = c.execute("""SELECT * FROM Children WHERE parent = ? and childEmail = ? """, (str(request.form['parent']),str(request.form['childEmail']))).fetchall()
            
            if Data:
               
                return 'already exist.'
            c.execute("""INSERT INTO Children (parent, childEmail, childName, childPassword , value) VALUES (?, ?,?,?,?)""", (str(request.form['parent']), str(request.form['childEmail']), str(request.form['childName']),  str(request.form['childPassword']),0.0))
            
            c.execute(
                    'insert into userPages (userName,pageName,urlPage) values(?,?,?);',
                    (str(request.form['childEmail']),str('Home'),str('/childuser'))
                    )
            
            c.execute(
                    'insert into userPages (userName,pageName,urlPage) values(?,?,?);',
                    (str(request.form['childEmail']),str('View Transaction'),str('/viewtransaction'))
                    )
            conn.commit()
    ...
    if request.form['action'] == "viewChild":
        user = request.form['parent']
        print(user)
        conn = sqlite3.connect("data.db")
        c = conn.cursor()
        Data = c.execute("""SELECT * FROM Children WHERE parent = ?  """, ([str(user)])).fetchall()
        print(Data)
        return jsonify({"loginOk":'true','data':Data})
        
    if request.form['action'] == "childPay":
        user = request.form['childEmail']
        conn = sqlite3.connect("data.db")
        c = conn.cursor()
        amount = c.execute("SELECT * FROM Children where childEmail = ?  ;",([user])).fetchall()[0][5]
        print(amount)
        ...
        time2 = datetime.strptime(request.form['creationTime'], "%H:%M:%S")
        thirty_minutes = timedelta(minutes=30)
        QRval = c.execute("""
            SELECT * from  QrValid where
         FromT = ? and ToT = ? and sec = ? and  min = ? and houre = ? and value = ? and transactionType = ? ;
            """,([data['childEmail'],data['mercent'],str(time2.second),str(time2.minute),str(time2.hour),str(data['value']),"local"])).fetchall()
        print(QRval)
        if QRval :
            flash('QR Code used before.')
            return 'QR Code used before.'
        if (time3 < time2 ) and ((time3 - time2) > thirty_minutes):
            flash('Invalid QR Code.')
            return 'Invalid QR Code.'
        if float(request.form['value']) > float(amount):
        ...
        c.execute("""INSERT INTO Transactions (FromT, ToT, cardNumber, CVV , expierDate,value,transactionType,actionTime) 
                    VALUES (?, ?, ?, ? , ?,?,?,?)"""
                    , (str(request.form['childEmail']), str(request.form['mercent']), "merchent payment",  "000",  "000",  str(request.form['value']),'Local Transaction',str(time1)))
        c.execute("UPDATE  Children  SET [value] =  [value] - ?   WHERE childEmail = ?",[(str(request.form['value'])),(str(request.form['childEmail']))])
        c.execute("""INSERT INTO QrValid (FromT, ToT, sec, min , houre,value,transactionType) 
                    VALUES (?, ?, ?, ? , ?,?,?)"""
                    , ([data['childEmail'],data['mercent'],str(time2.second),str(time2.minute),str(time2.hour),str(data['value']),"local"]))
        
        conn.commit()
        flash('Payment done Successfully.')
        return 'Payment done Successfully.'
    if request.form['action'] == "viewTransaction":
        user = request.form['username']
        conn = sqlite3.connect("data.db")
        c = conn.cursor()
        Data = c.execute("""SELECT * FROM Transactions WHERE FromT  = ? or ToT  = ?  """, (user ,user )).fetchall()
        print(Data)
        return jsonify({"loginOk":'true','data':Data})
    if request.form['action'] == "ChildMoney":
        user = request.form['username']
        conn = sqlite3.connect("data.db")
        c = conn.cursor()
        amount = c.execute("SELECT * FROM Children where childEmail = ?  ;",([user])).fetchall()[0][5]
    ...
    if request.form['action'] == "Login":
        ...
        if session.get('role') == "Money Depositor" :
            return jsonify(dataRes)
            ...
            dataRes = {
                "loginOK": "false",
                "massage":"bad usename or password"
                        } 
        ...
        data = {
        "loginOK": "false",
        "massage":"contact your administrator"
                }
...
def childuser():
    if ( session.get('role') == "child" ) and request.method == 'GET':
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS QrValid (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        FromT TEXT NOT NULL,
        ToT TEXT NOT NULL,
        sec TEXT NOT NULL,
        ...
        thirty_minutes = timedelta(minutes=5)
...
if __name__ == "__main__":
    ...
    if Table:
        ...
        time = datetime.now()
        c.execute("""INSERT INTO Users (username, password, name, roll , actiontime) VALUES (?, ?,?,?,?)""", ("armada@test.com", "123", "mohamed", "admin",str(time)))
        conn.commit()
    c.close()  
